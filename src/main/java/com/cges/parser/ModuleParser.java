...
public final class ModuleParser {
    ...
    public static ConcurrentGame<ModuleState<State>> parse(JsonObject json) {
        String gameName =
                requireNonNull(json.getAsJsonPrimitive("name"), "Missing name").getAsString();
        ...
        for (var agentEntry : requireNonNull(json.getAsJsonObject("modules"), "Missing modules definition")
                .entrySet()) {
            String moduleName = agentEntry.getKey();
            JsonObject agentData = agentEntry.getValue().getAsJsonObject();

            Formula moduleGoal = LtlParser.parse(
                            requireNonNull(agentData.getAsJsonPrimitive("goal"), () -> "Missing goal for module %s"
                                            .formatted(moduleName))
                                    .getAsString(),
                            propositions)
                    .formula();
            Agent.Payoff payoff = ParseUtil.parsePayoff(
                    requireNonNull(agentData.getAsJsonPrimitive("payoff"), () -> "Missing payoff for module %s"
                            .formatted(moduleName)));
            Set<String> moduleLabels = stream(
                            requireNonNull(agentData.getAsJsonArray("labels"), () -> "Missing labels for module %s"
                                    .formatted(moduleName)))
                    .map(JsonElement::getAsString)
                    .collect(Collectors.toSet());
            checkArgument(
                    propositionIndices.keySet().containsAll(moduleLabels),
                    "Module %s contains undefined labels %s",
                    moduleName,
                    Sets.difference(moduleLabels, propositionIndices.keySet()));
            Set<Action> actions = stream(
                            requireNonNull(agentData.getAsJsonArray("actions"), () -> "Missing actions for module %s"
                                    .formatted(moduleName)))
                    .map(JsonElement::getAsString)
                    .map(Action::new)
            ...
            State initialState =
                    new State(agentData.getAsJsonPrimitive("initial").getAsString());
            Map<State, BitSet> stateLabels = new HashMap<>();

            Map<State, Map<ModuleTransition<State>, BddSet>> transitions = new HashMap<>();
            for (Map.Entry<String, JsonElement> stateEntry :
                    agentData.getAsJsonObject("states").entrySet()) {
                ...
                Set<String> labels = stream(stateData.getAsJsonArray("labels"))
                        ...
                        .collect(Collectors.toSet());
                checkArgument(
                        moduleLabels.containsAll(labels),
                        "State %s of module %s labeled by %s",
                        stateName,
                        moduleName,
                        Sets.difference(labels, moduleLabels));
                ...
                for (JsonElement transitionData : modelTransitions) {
                    ...
                    BddSet valuation = transition.has("guard")
                            ? ParseUtil.parse(
                                    transition.getAsJsonPrimitive("guard").getAsString(), visitor)
                            : factory.of(true);
                    String actionName = transition.getAsJsonPrimitive("action").getAsString();
                    checkArgument(
                            actionName.equals("*") || agent.hasAction(actionName),
                            "State %s of module %s has uses unknown action %s",
                            stateName,
                            moduleName,
                            actionName);
                    (actionName.equals("*") ? agent.actions() : List.of(agent.action(actionName)))
                            .forEach(action -> stateTransitions.merge(
                                    new ModuleTransition<>(action, target), valuation, BddSet::union));
                }

                checkArgument(
                        stateTransitions.values().stream()
                                .reduce(BddSet::union)
                                .orElse(factory.of(false))
                                .isUniverse(),
                        "Incomplete transitions in state %s of module %s",
                        stateName,
                        moduleName);
                ...
                for (var entry : stateTransitions.entrySet()) {
                    actionTransitions.merge(entry.getKey().action(), entry.getValue(), (oldVs, newVs) -> {
                        checkArgument(
                                oldVs.intersection(newVs).isEmpty(),
                                "Transitions in state %s of module %s under action %s overlap",
                                stateName,
                                moduleName,
                                entry.getKey());
