    ...
    public static ConcurrentGame<ModuleState<State>> parse(JsonObject json) {
        ...
        for (var agentEntry : requireNonNull(json.getAsJsonObject("modules"), "Missing modules definition")
            ...
            Formula moduleGoal = LtlParser.parse(
                            requireNonNull(agentData.getAsJsonPrimitive("goal"),
                                            () -> "Missing goal for module %s".formatted(moduleName)).getAsString(),
            ...
            Set<String> moduleLabels = stream(requireNonNull(agentData.getAsJsonArray("labels"),
                            () -> "Missing labels for module %s".formatted(moduleName))).map(JsonElement::getAsString)
                            .collect(Collectors.toSet());
            checkArgument(propositionIndices.keySet().containsAll(moduleLabels),
                            "Module %s contains undefined labels %s", moduleName,
            ...
            Set<Action> actions = stream(requireNonNull(agentData.getAsJsonArray("actions"),
                            () -> "Missing actions for module %s".formatted(moduleName))).map(JsonElement::getAsString)
                            .map(Action::new).collect(Collectors.toUnmodifiableSet());
            ...
            for (Map.Entry<String, JsonElement> stateEntry : agentData.getAsJsonObject("states").entrySet()) {
                ...
                Set<String> labels = stream(stateData.getAsJsonArray("labels")).map(JsonElement::getAsString)
                                .collect(Collectors.toSet());
                checkArgument(moduleLabels.containsAll(labels), "State %s of module %s labeled by %s", stateName,
                                moduleName, Sets.difference(labels, moduleLabels));
                ...
                for (JsonElement transitionData : modelTransitions) {
                    ...
                    checkArgument(actionName.equals("*") || agent.hasAction(actionName),
                                    "State %s of module %s has uses unknown action %s", stateName, moduleName,
                                    actionName);
                    (actionName.equals("*") ? agent.actions() : List.of(agent.action(actionName)))
                                    .forEach(action -> stateTransitions.merge(new ModuleTransition<>(action, target),
                                                    valuation, BddSet::union));
                }

                checkArgument(stateTransitions.values().stream().reduce(BddSet::union).orElse(factory.of(false))
                                .isUniverse(), "Incomplete transitions in state %s of module %s", stateName,
                                moduleName);
                ...
                for (var entry : stateTransitions.entrySet()) {
                    actionTransitions.merge(entry.getKey().action(), entry.getValue(), (oldVs, newVs) -> {
                        checkArgument(oldVs.intersection(newVs).isEmpty(),
                                        "Transitions in state %s of module %s under action %s overlap", stateName,
            ...
            for (var transitionEntry : transitions.entrySet()) {
                for (var transition : transitionEntry.getValue().keySet()) {
                    checkArgument(stateLabels.containsKey(transition.destination()),
                                    ...
                                    transitionEntry.getKey().name(), moduleName, transition.action().name(),
