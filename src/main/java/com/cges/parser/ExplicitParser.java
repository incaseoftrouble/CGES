...
public final class ExplicitParser {
    ...
    public static ConcurrentGame<State> parse(JsonObject json) {
        ...
        Map<String, Agent> agents = requireNonNull(json.getAsJsonObject("agents"), "Missing agents definition")
                        .entrySet().stream().map(entry -> {
                            String agentName = entry.getKey();
                            JsonObject data = entry.getValue().getAsJsonObject();
                            Formula goal = LtlParser.parse(requireNonNull(data.getAsJsonPrimitive("goal"),
                                            () -> "Missing goal for agent %s".formatted(agentName)).getAsString(),
                                            propositions).formula();
                            Agent.Payoff payoff = ParseUtil
                                            .parsePayoff(requireNonNull(data.getAsJsonPrimitive("payoff"),
                            ...
                            Set<Action> actions = stream(requireNonNull(data.getAsJsonArray("actions"),
                                            ...
                                            .map(JsonElement::getAsString).map(Action::new)
        ...
        State initialState = new State(
                        requireNonNull(arena.getAsJsonPrimitive("initial"), "Missing initial adamState").getAsString());
        ...
        for (var stateEntry : requireNonNull(arena.getAsJsonObject("states"), "Missing states definition").entrySet()) {
            ...
            states.add(state);
            labels.putAll(state,
                            stream(requireNonNull(stateData.getAsJsonArray("labels"),
                                            () -> "Missing labels for state %s".formatted(stateName)))
                                            .map(JsonElement::getAsString).collect(Collectors.toSet()));
            transitions.putAll(state,
                            stream(requireNonNull(stateData.getAsJsonArray("transitions"),
                                            () -> "No transitions on state %s".formatted(stateName)))
                                            .map(JsonElement::getAsJsonObject).flatMap(transitionData -> {
                                                ...
                                                return Lists.cartesianProduct(actions).stream()
                                                                ...
                                                                .map(move -> new Transition<>(move, new State(
                                                                                transitionData.getAsJsonPrimitive("to")
                                                                                                .getAsString())));
                                            }).toList());
        }
        return new ExplicitGame<>(gameName, Set.copyOf(agents.values()), propositions, initialState, states.build(),
                        transitions.build(), labels.build()::get);
    }
