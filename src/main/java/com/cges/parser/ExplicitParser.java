...
public final class ExplicitParser {
    ...
    public static ConcurrentGame<State> parse(JsonObject json) {
        ...
        Map<String, Agent> agents =
                requireNonNull(json.getAsJsonObject("agents"), "Missing agents definition").entrySet().stream()
                        .map(entry -> {
                            String agentName = entry.getKey();
                            JsonObject data = entry.getValue().getAsJsonObject();
                            Formula goal = LtlParser.parse(
                                            requireNonNull(
                                                            data.getAsJsonPrimitive("goal"),
                                                            () -> "Missing goal for agent %s".formatted(agentName))
                                                    .getAsString(),
                                            propositions)
                                    .formula();
                            Agent.Payoff payoff = ParseUtil.parsePayoff(requireNonNull(
                                    data.getAsJsonPrimitive("payoff"),
                                    () -> "Missing payoff for agent %s".formatted(agentName)));
                            Set<Action> actions = stream(requireNonNull(
                                            data.getAsJsonArray("actions"),
        ...
        State initialState = new State(requireNonNull(arena.getAsJsonPrimitive("initial"), "Missing initial adamState")
                .getAsString());
        ...
        for (var stateEntry : requireNonNull(arena.getAsJsonObject("states"), "Missing states definition")
                .entrySet()) {
            ...
            labels.putAll(
                    state,
                    stream(requireNonNull(stateData.getAsJsonArray("labels"), () -> "Missing labels for state %s"
                                    .formatted(stateName)))
                            .map(JsonElement::getAsString)
                            .collect(Collectors.toSet()));
            transitions.putAll(
                    state,
                    stream(requireNonNull(stateData.getAsJsonArray("transitions"), () -> "No transitions on state %s"
                                    .formatted(stateName)))
                            ...
                            .flatMap(transitionData -> {
                                ...
                                for (var entry : requireNonNull(
                                                transitionData.getAsJsonObject("actions"),
                                                () -> "Missing actions for transition on state %s".formatted(stateName))
                                        .entrySet()) {
                                    ...
                                    actions.add(
                                            action.equals("*")
                                                    ? List.copyOf(agent.actions())
                                                    : List.of(agent.action(action)));
                                }
                                return Lists.cartesianProduct(actions).stream()
                                        .map(pureMove -> IntStream.range(0, agents.size())
                                                .boxed()
                                                .collect(Collectors.toMap(actionAgents::get, pureMove::get)))
                                        .map(ExplicitGame.MapMove::new)
                                        .map(move -> new Transition<>(
                                                move,
                                                new State(transitionData
                                                        .getAsJsonPrimitive("to")
                                                        .getAsString())));
                            })
                            .toList());
        }
