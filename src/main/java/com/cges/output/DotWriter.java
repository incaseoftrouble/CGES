...
public final class DotWriter {
    ...
    public static <S> void writeModule(Module<S> module, ModuleGame<?> game, PrintStream writer) {
        ...
        for (var entry : ids.object2IntEntrySet()) {
            ...
            for (Action action : module.actions(state)) {
                for (Map.Entry<S, BddSet> transition :
                        module.successorMap(state, action).entrySet()) {
                    successorByAction
                            .computeIfAbsent(transition.getKey(), k -> new HashMap<>())
                            .merge(action, transition.getValue(), BddSet::union);
                }
            }
    ...
    public static <S> void writeHistoryGame(
            HistoryGame<S> game, PrintStream writer, @Nullable Predicate<HistoryState<S>> winning) {
            ...
            for (var moveEntry : movesBySuccessor.asMap().entrySet()) {
                writer.append("HS_%d -> HS_%d [label=\"%s\"]\n"
                        .formatted(
                                ...
                                moveEntry.getValue().stream()
                                        .map(DotFormatted::toDotString)
                                        .collect(Collectors.joining(", "))));
            }
        }
        writer.append("}");
    ...
    public static <S> void writeSuspectGame(
            SuspectGame<S> game,
            SuspectGame.EveState<S> initial,
            PrintStream writer,
    ...
    public static <S> void writeLasso(GameSolution<S> solution, PrintStream writer) {
                                    ...
                                    suspectGame.historyGame().concurrentGame().agents().stream()
                                            ...
                                            .map(a -> "%s %s"
                                                    .formatted(
                                                            ...
                                                            DotFormatted.toRecordString(LabelledFormula.of(
                                                                            historyState.goal(a), atomicPropositions)
                                                                    .toString())))),
        ...
        ids.forEach((runState, id) -> {
            ...
            var historySuccessor = suspectGame
                    .historyGame()
                    .transition(runState.historyState(), move)
                    .orElseThrow()
                    .destination();
            var runSuccessor = runGraph.successors(runState).stream()
                    .filter(s -> s.historyState().equals(historySuccessor))
                    .findAny()
                    .orElseThrow();
    ...
    public static <S> void writeSolution(GameSolution<S> solution, PrintStream writer) {
        ...
        Set<PriorityState<S>> reachableGameStates = loopStates.stream()
                .flatMap(runState -> punishmentStrategy.states(runState.historyState(), moves.get(runState)).stream()
                        .filter(s -> !s.eve()
                                .historyState()
                                .equals(lasso.successor(runState).historyState())))
        ...
        gameIds.forEach((gameState, id) -> {
            if (gameState.isEve()) {
                String eveState = Stream.concat(
                                Stream.of(toDotString(gameState.eve().gameState())),
                                gameState.eve().suspects().stream()
                                        .sorted(Comparator.comparing(Agent::name))
                                        .map(a -> "%s: %s"
                                                .formatted(
                                                        a.name(),
        ...
        loopIds.forEach((runState, id) -> {
            ...
            punishmentStrategy.states(runState.historyState(), move).stream()
                    .filter(s -> !s.eve()
                            .historyState()
                            .equals(lasso.successor(runState).historyState()))
                    .forEach(punishmentState -> writer.append(
                            "HS_%d -> GS_%d [style=dotted];\n".formatted(id, gameIds.getInt(punishmentState))));
        });
        gameIds.forEach((gameState, id) -> {
