...
public class HistoryGame<S> {
  ...
  public HistoryGame(ConcurrentGame<S> game) {
    this.game = game;
  ...
  public Stream<Transition<HistoryState<S>>> transitions(HistoryState<S> state) {
    checkArgument(state instanceof HistoryGame.ListHistoryState<S>);
    ...
    List<EquivalenceClass> successorGoals = Lists.transform(history.agentGoals(), f -> f.temporalStep(valuation));
