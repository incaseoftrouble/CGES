...
public final class SuspectParityGame<S> implements ParityGame<PriorityState<S>> {
  ...
  private SuspectParityGame(SuspectGame<S> suspectGame, EveState<S> initialState, Automaton<Object, ParityAcceptance> dpa) {
    ...
    Set<NonDeviationState<S>> nonDeviationStates = new HashSet<>(List.of(
        new NonDeviationState<>(historyGame.initialState(), dpa.initialState())));
    Queue<NonDeviationState<S>> nonDeviationQueue = new ArrayDeque<>(nonDeviationStates);
    ImmutableSetMultimap.Builder<HistoryGame.HistoryState<S>, NonDeviationState<S>> historyStateMap = ImmutableSetMultimap.builder();
    ImmutableTable.Builder<NonDeviationState<S>, Move, Set<PriorityState<S>>> deviationSuccessors = ImmutableTable.builder();
    ...
    Set<PriorityState<S>> reached = this.deviationSuccessors.values().stream().flatMap(Collection::stream).collect(Collectors.toSet());
    ...
    while (!queue.isEmpty()) {
      ...
      eveState.suspects().stream().map(Agent::name)
          .mapToInt(propositionIndex::getInt)
          .filter(i -> i >= 0)
          .forEach(label::set);
